...
import { Activity, User, Lock, Phone, Calendar, Briefcase, Stethoscope, ArrowRight, Sparkles, CheckCircle2, Shield, Zap, Layers } from 'lucide-react';
      ...
      <div className="w-full lg:w-1/2 p-6 sm:p-12 lg:p-24 flex flex-col justify-between relative z-10 overflow-y-auto custom-scrollbar">
        ...
        <div className="max-w-xl w-full mx-auto lg:mx-0">
          ...
          >
            ...
            <p className="text-gray-500 font-bold text-sm uppercase tracking-widest mb-12">Initialize your medical profile.</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="grid grid-cols-2 gap-4">
                {['Patient', 'Doctor'].map((role) => (
                  ...
                  >
                    {role === 'Patient' ? <User className={`w-8 h-8 ${formData.role === 'Patient' ? 'text-blue-600' : 'group-hover:text-white'}`} /> : <Stethoscope className={`w-8 h-8 ${formData.role === 'Doctor' ? 'text-blue-600' : 'group-hover:text-white'}`} />}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <AnimatePresence mode="wait">
                  {formData.role === 'Doctor' ? (
                    ...
                    >
                      <div className="space-y-2">
                        <label className="text-[10px] font-bold text-gray-600 uppercase tracking-[0.3em] ml-4">Specialization</label>
                        <input
                          type="text"
                          name="specialization"
                          required
                    ...
                    >
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] font-bold text-gray-600 uppercase tracking-[0.3em] ml-4">Preferred Dept</label>
                        <select
                          name="department_id"
                          value={formData.department_id}
                          onChange={handleChange}
            ...
            <p className="mt-12 text-center lg:text-left text-[10px] font-bold text-gray-600 uppercase tracking-[0.2em]">
              Already registered? <Link to="/login" className="text-blue-500 hover:underline underline-offset-4">Access Portal</Link>
            </p>
          </motion.div>
        </div>
      </div>
      ...
      <div className="hidden lg:flex w-1/2 bg-white/2 border-l border-white/5 relative items-center justify-center p-24 overflow-hidden">
        <div className="relative z-10 text-center">
          ...
          <h2 className="text-5xl font-display font-black uppercase tracking-tighter mb-8 leading-none">
            Unified <br />
            <span className="text-transparent stroke-text">Infrastructure</span>
          </h2>
          <p className="text-gray-500 text-sm max-w-sm mx-auto leading-relaxed uppercase tracking-widest">
            "A seamless infrastructure connecting every department, ward, and patient record in real-time."
          </p>
          <div className="mt-12 flex items-center justify-center gap-8">
            <div className="text-center">
              <p className="text-2xl font-display font-bold text-white leading-none mb-1">99%</p>
              <p className="text-[8px] font-bold uppercase tracking-widest text-gray-600">Accuracy</p>
            </div>
            <div className="w-px h-8 bg-white/10" />
            <div className="text-center">
              <p className="text-2xl font-display font-bold text-white leading-none mb-1">0.1s</p>
              <p className="text-[8px] font-bold uppercase tracking-widest text-gray-600">Latency</p>
            </div>
          </div>
        </div>
